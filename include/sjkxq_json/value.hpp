...
public:
    ...
    Value(std::string&& value) : type_(ValueType::String), string_(new std::string(std::move(value))) {}
    ...
    int as_int() const {
        if (!is_number()) throw std::runtime_error("Value is not a number");
        double intpart;
        if (std::modf(number_, &intpart) != 0.0 || number_ > static_cast<double>(INT_MAX) || number_ < static_cast<double>(INT_MIN))
            throw std::runtime_error("Value is not an integer or out of range");
        return static_cast<int>(intpart);
    }

    const std::string& as_string() const {
        if (!is_string() || !string_) throw std::runtime_error("Value is not a string");
        return *string_;
    }

    const Array& as_array() const {
        if (!is_array() || !array_) throw std::runtime_error("Value is not an array");
        return *array_;
    }

    const Object& as_object() const {
        if (!is_object() || !object_) throw std::runtime_error("Value is not an object");
        return *object_;
    }
